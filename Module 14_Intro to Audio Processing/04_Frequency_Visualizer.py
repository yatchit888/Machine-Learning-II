...
while 1:  # for each recorded window (until ctr+c) is pressed
    ...
    # plt.figure()
    # plt.barh(X2, width=max_width_from_energy, height=100)
    # # plt.bar(X2, labels=[str(int(f)) + " Hz" for f in freqs2[0:-1]],
    # #          show_vals=False, width=max_width_from_energy, height=100)
    # plt.show()
